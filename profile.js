...
function getFlag(user) {
	...
	if (Users(user)) {
		let flag = geoip.lookupCountry(Users(user).latestIp);
		if (!flag) {
			return '';
		} else {
			return ' <img src="https://github.com/kevogod/cachechu/blob/master/flags/' + flag.toLowerCase() + '.png?raw=true" height=10 title="' + flag + '">';
		}
	}
}
...
Profile.prototype.buttonAvatar = function () {
	return '<button style="border: none; background:none; padding: 0; float: left;" name="parseCommand" value="/user ' + this.username + '">' + this.avatar() + "</button>";
};
...
Profile.prototype.show = function (callback) {
	...
	return this.buttonAvatar() +
		SPACE + this.name() + SPACE + getFlag(userid) + BR +
		SPACE + this.group() + BR +
